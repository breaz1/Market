...
import { APartners } from './adminPartners'
...
const AdminPartners = () => {
  ...
  const fetchPartnersData = async () => {
    try {
      ...
      setPartnersData(response.data) // Save the partners data in state
    } catch (error) {
      console.log(error)
    }
  }
  ...
  const OnSubmit = (event) => {
    ...
    axios.post('', { name, content: event.target.file.files[0] }, {
      headers: {
        ...
        <div>
            <form className='admin-form' onSubmit={OnSubmit}>
                <FormGroup className='admin-form'>
                    <h1 className="auth-title">Добавление Партнеров</h1>
                    ...
                    <button className="admin-confirm-button">Добавить партнера</button>
                </FormGroup>
            </form>
            <div className="admin-catalog catalog-catalog">
                <div className="">
                    ...
                    {partnersData.map((partner) =>
                        <APartners key={partner._id} data={partner} />
                    )}
                </div>
            </div>
            {showOrder && (
                     <div className="notification slideIn">Партнер добавлен</div>
            )}
        </div>
  )
}
